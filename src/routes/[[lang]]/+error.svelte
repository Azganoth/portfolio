...
>
  ...
  <h1
    class="font-orbitron mb-8 text-3xl font-bold tracking-wide md:text-4xl"
    {@attach reveal()}
  >
    {title}
  </h1>
  <p class="text-muted-foreground text-lg" {@attach reveal()}>
    {description}
  </p>
  ...
  <div class="reveal-slide-up duration-700" {@attach reveal()}>
    <Link
      ...
      class="bg-secondary hover:bg-secondary/90 font-jetbrains-mono text-background mt-24 block rounded-full px-6 py-3 transition-colors"
      variant="none"
    >
      {$t("error_back_home")}
    </Link>
