...
<Section id={ID_SKILLS_SECTION} title={t("skills_title")}>
  <div
    class="grid grid-cols-1 gap-10 md:grid-cols-2 xl:grid-cols-3 xl:gap-y-16"
  >
    {#each categorizedSkills as { category, skills } (category)}
      <div>
        <h3
          class="relative mb-8 text-center text-lg font-semibold after:absolute after:-bottom-4 after:center-x after:h-1 after:w-8 after:rounded-full after:bg-primary after:content-['']"
        >
          {category}
        </h3>
        <ul class="flex flex-wrap justify-center gap-4">
          {#each skills as skill, i (skill)}
            ...
            >
              <div
                class="group flex items-center gap-3 rounded-lg border border-white/10 bg-white/5 p-3 text-sm transition-transform ease-snappy hover:-translate-y-1"
              >
                <Icon
