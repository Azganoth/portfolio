...
<Section id={ID_SKILLS_SECTION} title={t("skills_title")}>
  <dl class="mt-12 flex flex-col">
    {#each skillGroups as { title, skills }, groupIndex (title)}
      <Reveal
        class="grid gap-x-7 gap-y-3 border-t border-white/8 py-6 delay-(--delay) duration-500 ease-out-expo not-in-view:translate-y-4 not-in-view:opacity-0 md:grid-cols-[9rem_minmax(0,1fr)] md:items-baseline"
        style={`--delay: ${groupIndex * 80}ms`}
      >
        <dt
          class="font-mono text-xs font-bold tracking-widest text-secondary uppercase"
        >
          {title}
        </dt>
        <dd class="m-0">
          <ul class="flex flex-wrap gap-x-3 gap-y-2.5">
            {#each skills as skill (skill)}
              ...
              <li
                class="group/skill flex items-center gap-2 rounded-lg border border-white/10 bg-white/4 px-2.5 py-2.5 text-sm transition-colors select-none hover:border-white/20 hover:bg-white/8"
              >
                <SkillIcon
                ...
                />
                <span
                  class="font-mono font-semibold tracking-wide text-foreground/85"
                >
                  {skill}
                </span>
              </li>
