<script lang="ts" module>
  export const getTitleId = (name: string) => `project-${name}-title`;
  export const getSummaryId = (name: string) => `project-${name}-summary`;
</script>

<script lang="ts">
  import { TECH_META } from "@app/constants";
  import { activeProject } from "@app/store";
  import { PROJECT_DETAILS_ID } from "@components/interactive/ProjectDetails.svelte";
...
<button
  ...
  aria-labelledby={getTitleId(project.name)}
  aria-describedby={getSummaryId(project.name)}
